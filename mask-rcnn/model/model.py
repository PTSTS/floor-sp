...
class ResNet(nn.Module):
    ...
    def __init__(self, architecture, input_channel=3, stage5=False):
...
def proposal_layer(inputs, proposal_count, nms_threshold, anchors, config=None):
    ...
    order = order[:, :pre_nms_limit].unsqueeze(-1).repeat(1, 1, 4)
    scores = scores[:, :pre_nms_limit]
    anchors = anchors.unsqueeze(0).repeat(order.shape[0], 1, 1)
    deltas = torch.gather(deltas, 1, order.data)
    anchors = torch.gather(anchors, 1, order.data)
...
def pyramid_roi_align(inputs, pool_size, image_shape):
    ...
    for i, level in enumerate(range(2, 6)):
        ...
        ix = torch.nonzero(ix)
        ix = ix.detach()
        level_boxes = boxes[ix[:, 0].data, ix[:, 1].data, :]
    ...
    feat_shape = pooled.shape[1:]

    # Pack box_to_level mapping into one array and add another
    # column representing the order of pooled boxes
    box_to_level = torch.cat(box_to_level, dim=0)
    box_to_level = N * box_to_level[:, 0] + box_to_level[:, 1]

    # Rearrange pooled features to match the order of the original boxes
    _, box_to_level = torch.sort(box_to_level)
    pooled = pooled[box_to_level]  # .view(-1, N, *feat_shape)
...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    if positive_count > 0 or negative_count > 0:
        rois = torch.cat((positive_rois, negative_rois), dim=0) if negative_count > 0 else positive_rois
        zeros = Variable(torch.zeros(negative_count), requires_grad=False).int()
        if config.GPU_COUNT:
            zeros = zeros.cuda()
        roi_gt_class_ids = torch.cat([roi_gt_class_ids, zeros])
        N = negative_count
        P = max(config.TRAIN_ROIS_PER_IMAGE - rois.shape[0], 0.0)
        rois = F.pad(rois, (0, 0, 0, P), "constant", 0.0)
        roi_gt_class_ids = F.pad(roi_gt_class_ids, (0, P), "constant", -1)
        deltas = F.pad(deltas, (0, 0, 0, N + P), "constant", 0.0)
        masks = F.pad(masks, (0, 0, 0, 0, 0, N + P), "constant", 0.0)
    else:
        rois = torch.zeros((config.TRAIN_ROIS_PER_IMAGE, 4)).cuda()
        roi_gt_class_ids = torch.zeros((config.TRAIN_ROIS_PER_IMAGE)).int().cuda() - 1
        deltas = torch.zeros((config.TRAIN_ROIS_PER_IMAGE, 4)).cuda()
        masks = torch.zeros((config.TRAIN_ROIS_PER_IMAGE, config.MASK_SHAPE[0], config.MASK_SHAPE[1])).cuda()
...
def refine_detections(rois, probs, deltas, window, config):
    ...
    if config.GPU_COUNT:
        std_dev = std_dev.cuda()
    refined_rois = apply_box_deltas(rois.unsqueeze(0), (deltas_specific * std_dev).unsqueeze(0)).squeeze(0)
...
def relation_layer(detections, scale):
    # Grab rois and ids
    boxes = detections[:, :4]
    ids = detections[:, 4]

    # Select corners and edges
    corners_ind = torch.nonzero(ids == 2).squeeze(-1)
    edges_ind = torch.nonzero(ids == 1).squeeze(-1)

    if corners_ind.size()[0] and edges_ind.size()[0]:
        corners_rois = boxes[corners_ind.data, :] / scale
        edges_rois = boxes[edges_ind.data, :] / scale
    else:
        corners_rois = Variable(torch.FloatTensor(), requires_grad=False)
        edges_rois = Variable(torch.FloatTensor(), requires_grad=False)
    return corners_rois, edges_rois
...
class Orientation(nn.Module):
    def __init__(self, depth, pool_size, image_shape, n_bins=18):
        ...
        self.bn2 = nn.BatchNorm2d(1024, eps=0.001, momentum=0.01)
        self.relu = nn.ReLU(inplace=True)
        self.linear_class = nn.Linear(1024, n_bins)
    ...
    def forward(self, x, rois):
        ...
        x = self.relu(x)
        x = x.view(-1, 1024)
        edges_dir_logits = self.linear_class(x)

        return edges_dir_logits
...
class Encoder(nn.Module):
    def __init__(self, depth, pool_size, image_shape):
        ...
        self.bn2 = nn.BatchNorm2d(1024, eps=0.001, momentum=0.01)
        self.relu = nn.ReLU(inplace=True)
        self.linear_class = nn.Linear(1024, 512)
...
class Relation(nn.Module):
    def __init__(self):
        ...
        self.fc_ce = nn.Linear(1024, 512, bias=True)
        self.fc_ce2 = nn.Linear(512, 512, bias=True)
        self.fc_ce3 = nn.Linear(512, 1, bias=True)

    def forward(self, cx, ex):
        n_c = cx.shape[0]
        n_e = ex.shape[0]
        cx = cx.unsqueeze(1).repeat(1, n_e, 1)
        ex = ex.unsqueeze(0).repeat(n_c, 1, 1)
        x = F.relu(self.fc_ce(torch.cat([cx, ex], dim=-1)))
        x = F.relu(self.fc_ce2(x))
        probs = F.sigmoid(self.fc_ce3(x))
        return probs
...
def compute_rpn_class_loss(rpn_match, rpn_class_logits):
    ...
    # Reshape
    rpn_match = rpn_match.view(-1)
    rpn_class_logits = rpn_class_logits.view(-1, *rpn_class_logits.shape[2:])
    ...
    # Crossentropy loss
    loss = F.cross_entropy(rpn_class_logits, anchor_class, ignore_index=-1)
...
def compute_rpn_bbox_loss(target_bbox, rpn_match, rpn_bbox):
    ...
    y_true = []
    y_pred = []
    for rm, rb, tg_bb in zip(rpn_match, rpn_bbox, target_bbox):
        # Positive anchors contribute to the loss, but negative and
        # neutral anchors (match value of 0 or -1) don't.
        indices = torch.nonzero(rm == 1)

        # Pick bbox deltas that contribute to the loss
        rb = rb[indices.data[:, 0], :]

        # Trim target bounding box deltas to the same length as rpn_bbox.
        tg_bb = tg_bb[:rb.size()[0], :]
...
def compute_edges_dir_loss(target_edges_dir, edges_dir, n_bins=18):
    # Reshape
    target_edges_dir = target_edges_dir.view(-1)
    edges_dir = edges_dir.view(-1, n_bins)

    # Filter neutral elements (-1)
    keep = torch.nonzero(target_edges_dir >= 0).squeeze(-1)
    if keep.size()[0]:
        target_edges_dir = target_edges_dir[keep.data]
        edges_dir = edges_dir[keep.data, :]
        loss = F.cross_entropy(edges_dir, target_edges_dir.long())
    else:
        loss = Variable(torch.FloatTensor([0]), requires_grad=False)
        if target_edges_dir.is_cuda:
            loss = loss.cuda()

    return loss


def compute_relation_loss(relations_target, relations_prob):
    # Reshape
    relations_prob = relations_prob.squeeze(-1)

    if relations_target.size()[0]:
        loss = F.binary_cross_entropy(relations_prob, relations_target)
    else:
...
def compute_mrcnn_class_loss(target_class_ids, pred_class_logits):
    ...
    # Reshape
    pred_class_logits = pred_class_logits.view(-1, pred_class_logits.shape[-1])
    target_class_ids = target_class_ids.view(-1)

    # Filter padded elements
    ind = torch.nonzero(target_class_ids >= 0).squeeze(-1)
    target_class_ids = target_class_ids[ind.data]
    pred_class_logits = pred_class_logits[ind.data, :]
...
def compute_mrcnn_bbox_loss(target_bbox, target_class_ids, pred_bbox):
    ...
    """
    target_bbox = target_bbox.view(-1, 4)
    target_class_ids = target_class_ids.view(-1)

    # Filter padded elements
    positive_roi_ix = torch.nonzero(target_class_ids > 0).long()
    positive_roi_class_ids = target_class_ids[positive_roi_ix.data].long()

    if positive_roi_class_ids.size()[0]:
...
def compute_mrcnn_mask_loss(target_masks, target_class_ids, pred_masks):
    ...
    # Reshape
    target_masks = target_masks.view(-1, *target_masks.shape[2:])
    target_class_ids = target_class_ids.view(-1)

    # filter emtpy masks
    keep = torch.nonzero(torch.sum(target_masks.view(target_masks.shape[0], -1), dim=-1) > 0)
    if keep.size()[0]:
        target_masks = target_masks[keep.data[:, 0], :, :]
        pred_masks = pred_masks[keep.data[:, 0], :, :, :]
        target_class_ids = target_class_ids[keep.data[:, 0]]
        positive_ix = torch.nonzero(target_class_ids > 0)[:, 0]

    if keep.size()[0] and positive_ix.size()[0]:
        # Only the class specific mask of each ROI.
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, image_index):
        ...
        image, image_metas, gt_class_ids, gt_boxes, gt_masks = load_image_gt(self.dataset, self.config, image_id,
...
class MaskRCNN(nn.Module):
    ...
    def __init__(self, config, model_dir, input_channel):
        """
    ...
    def build(self, config):
        ...
        resnet = ResNet("resnet101", input_channel=self.input_channel, stage5=True)
    ...
    def load_pretrained_weights(self, filepath, extra_channels=3):
        ...
        if os.path.exists(filepath):
            ...
            for k, v in pretrained_dict.items():
                ...
                elif 'rpn' in k and ('class' in k or 'bbox' in k):
                    num_ratios = len(self.config.RPN_ANCHOR_RATIOS)
                    assert num_ratios % 2 == 1
                    extra_num = (num_ratios - 3) // 2
                    if extra_num > 0:
                        cof = 2 if 'class' in k else 4
                        left_pad_weights = torch.cat([v[:cof].clone() for _ in range(extra_num)], dim=0)
                        right_pad_weights = torch.cat([v[2 * cof:].clone() for _ in range(extra_num)], dim=0)
                        new_weight = torch.cat([left_pad_weights, v, right_pad_weights], dim=0)
                        new_weights.update({k: new_weight})
                    else:
                        new_weights.update({k: v})
    ...
    def detect(self, images):
        ...
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_masks = \
                self.unmold_detections(detections[i], mrcnn_mask[i], image.shape, windows[i])
            results.append({
                "rois": final_rois,
    ...
    def predict(self, input, mode):
        ...
        # Adjust the inputs to RPN, since we might add extra scales
        min_idx = self.config.RPN_ANCHOR_SCALES.index(16)
        max_idx = self.config.RPN_ANCHOR_SCALES.index(256)
        if min_idx != 0:
            for i in range(min_idx):
                rpn_feature_maps = [p2_out, ] + rpn_feature_maps
        if max_idx != len(self.config.RPN_ANCHOR_SCALES) - 1:
            for i in range(max_idx - len(self.config.RPN_ANCHOR_SCALES) + 1):
                rpn_feature_maps.append(p6_out)
        ...
        if mode == 'inference':
            ...
            # TODO: put loop over batch inside function
            rois, target_class_ids, target_deltas, target_mask = [], [], [], []
            for k in range(rpn_rois.shape[0]):
                rois_b, target_class_ids_b, target_deltas_b, target_mask_b = \
                    detection_target_layer(rpn_rois[k], gt_class_ids[k], gt_boxes[k], gt_masks[k], self.config)
                rois.append(rois_b)
                target_class_ids.append(target_class_ids_b)
                target_deltas.append(target_deltas_b)
                target_mask.append(target_mask_b)

            rois = torch.stack(rois, dim=0)
            target_class_ids = torch.stack(target_class_ids, dim=0)
            target_deltas = torch.stack(target_deltas, dim=0)
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs, layers, config=None):
        """Train the model.
        ...
        train_generator = torch.utils.data.DataLoader(train_set, batch_size=config.TRAIN_BATCH_SIZE, shuffle=True,
                                                      num_workers=1, collate_fn=PadCollate())
    ...
    def train_epoch(self, datagenerator, optimizer, steps):
        ...
        for inputs in datagenerator:
            ...
            rpn_class_logits, rpn_pred_bbox, target_class_ids, mrcnn_class_logits, target_deltas, mrcnn_bbox, target_mask, mrcnn_mask = \
                self.predict([images, image_metas, gt_class_ids, gt_boxes, gt_masks],
                             mode='training')
    ...
    def evaluate_map(self, test_dataset, image_list, vocabulary, with_depth=False, image_list_depth=None):
        ...
        if with_depth:
            assert image_list_depth

        for i_data, data in enumerate(val_generator):
            image = image_list[i_data]
            if with_depth:
                depth_image = image_list_depth[i_data]
                image = np.concatenate([image, np.expand_dims(depth_image, axis=-1)], axis=-1)
            gt_class_ids = data[4].squeeze(0).numpy()
            gt_edge_dirs = data[5].squeeze(0).numpy()
            gt_boxes = data[7].squeeze(0).numpy()
            gt_masks = data[8].squeeze(0).numpy()

            roi_info = {
                'boxes': gt_boxes,
                'masks': gt_masks,
                'gt_classes': gt_class_ids,
                'gt_edge_dirs': gt_edge_dirs
            }
        ...
        all_dets = [[[] for _ in range(num_images)] for _ in range(num_classes)]
        ...
        for i in range(num_images):
            ...
            img_edges_dir = detection_results[i]['edges_dir']
            for j in range(1, num_classes):
                ...
                for det_i, class_id in enumerate(img_class_ids):
                    if class_id == j:
                        if class_id == 1:
                            dets.append(img_boxes[det_i])
                            scores.append(img_scores[det_i])
                            edge_dirs.append(img_edges_dir[det_i])
                        elif class_id == 2:
                            y1, x1, y2, x2 = img_boxes[det_i]
                            dets.append(np.array([(y2 + y1) / 2.0, (x2 + x1) / 2.0, -1, -1]))
                            scores.append(img_scores[det_i])
                ...
                if len(dets) > 0:
                    ...
                    if j == 1:
                        edge_dirs = np.stack(edge_dirs, axis=0)
                        all_dets[j][i] = np.concatenate([dets, np.expand_dims(scores, axis=1), edge_dirs], axis=1)
                    elif j == 2:
                        all_dets[j][i] = np.concatenate([dets, np.expand_dims(scores, axis=1)], axis=1)
                else:
                    all_dets[j][i] = empty_array

        utils.primitive_detector_eval(all_dets, test_roidb, vocabulary=vocabulary)
